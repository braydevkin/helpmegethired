...
| `GET /profile/curation` | The current Curation, the newest one of the latest completed Profile that is not `superseded`: status, the percentage `floor(100 × saved units ÷ units)`, the total and saved counts, every unit in order with its kind, title, and status (the units being read are the `running` ones, up to three at once), the computed facts, the Model in use, the failure reason with `resume_after`, and `rerun`: whether a re-run is `allowed`, and the `refusal` code `POST /profile/curation/rerun` would answer (`curation_active`, `curation_not_ready`, or `curation_unchanged`); an `ETag` over the whole answer, so a change of Model Choice that makes a re-run available changes it and `304` on `If-None-Match`; `{ "progress": null }`, not a `404`, when the Account has no such Curation. Statements are not part of it |
