...
- `/journey` is the first authenticated page and opens the step the Candidate is on (`journeyStepOf`): the résumé upload until an Ingestion has built a Profile, the Profile review until the Candidate confirms it, then a redirect to `/journey/analysis`. `/journey/resume`, `/journey/profile`, `/journey/ai`, and `/journey/analysis` are the steps' own routes, and confirming the Profile redirects to the analysis. Every journey page asks the API for the Account behind the cookie (`requireCandidate`) and redirects to `/sign-in` when the API no longer accepts the token. Sign out is a server action that deletes the Session at the API, clears the cookie, and redirects to `/sign-in`.
...
- The summary's primary action goes to `/journey/analysis`. Until a key is stored it is a disabled button with its reason beside it; once one is, its hint says the tab can be closed, with no duration.
