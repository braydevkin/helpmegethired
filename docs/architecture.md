...
- `/journey` is the first authenticated page and opens the step the Candidate is on (`journeyStepOf`, from the Profile and `GET /account/model` read together): a redirect to `/journey/ai` until a Model Key is stored, because the Candidate's own Model reads the résumé at the upload (this holds for an Account whose Profile was built before that order too); then the résumé upload until an Ingestion has built a Profile, the Profile review until the Candidate confirms it, then a redirect to `/journey/analysis`. `/journey/resume`, `/journey/profile`, `/journey/ai`, and `/journey/analysis` are the steps' own routes, and confirming the Profile redirects to the analysis. Every journey page asks the API for the Account behind the cookie (`requireCandidate`) and redirects to `/sign-in` when the API no longer accepts the token. Sign out is a server action that deletes the Session at the API, clears the cookie, and redirects to `/sign-in`.
- The Profile step reads the Profile on the server and renders the review as the design has it, with the Candidate's corrections on top of it (#178). The headline and the two URLs are read in the header and the sidebar; `BasicProfileCard` shows the summary and corrects all four fields, and `ExperienceCorrections` puts `Correct this role`, `Remove` and `Add a role` on the timeline. `ProfileEditingProvider` keeps one correction open across the whole step, holds `Confirm profile` back while one is, and hands the focus back to the control that opened it. The forms are corrections of the Experience the API answered, never of the period the timeline writes for reading, and `correction-form.ts` turns the fields back into the shared schemas before anything is sent, so a message lands on the field it belongs to; the forms submit from `onSubmit`, because React resets a form after an `action` settles and a refused correction keeps what was typed. Every save is a server action that calls `revalidatePath` and answers only whether it was saved, so the step re-renders from the Profile the API reads again and never from local state. A confirmed Profile renders no correction because `profileViewOf` makes it not `correctable`, and says why beside `Profile confirmed`; the write routes answering `409` is the guard behind it.
...
- Three server actions in `src/app/(journey)/journey/resume/actions.ts` are the page's only way to the API, through `ResumeClient`: `createResumeAction` parses `ResumeUploadSchema` and calls `POST /resumes` with the SHA-256 computed in the browser (`crypto.subtle`), `completeResumeAction` calls `POST /resumes/:id/complete`, and `readResumeAction` calls `GET /resumes/:id` with the last `ETag` as `If-None-Match`, answering "unchanged" on `304`. A `409` becomes the design's copy (`ingestion_active`, `upload_incomplete`, `model_key_missing`), and the failure keeps its `ResumeRefusalCode`; the Session token never reaches the browser.
- The server component also reads `GET /account/model`. With no key stored, the idle state is `ModelChoiceBeforeUpload`, a gate that leads to `/journey/ai` and keeps the upload as a locked entry with its reason, instead of the drop zone; a record already shown on load stays shown. A `409 model_key_missing` from `POST /resumes`, as after a key revoked in another tab, switches the page to the same gate.
...
### Choose your AI (FR-12, TC-09)

The provider and key step at `/journey/ai` follows [Design: Choose your AI](https://github.com/braydevkin/helpmegethired/wiki/Design-Choose-your-AI) within the phase-one scope of ADR-0023: Anthropic and the pinned model of the shared catalogue, no index selector, and no cost, speed, or quality figure anywhere. The server component reads `GET /account/model` and renders the `ModelChoiceSetup` organism through `ModelChoiceFlow`, a client component next to the page.

- The Model Key never passes through the web app. On submit, `ModelChoiceFlow` parses `ModelChoiceRequestSchema` in the browser, asks `requestModelKeyTicketAction` for a Model Key ticket (the server action calls `POST /account/model/key-ticket` with the Session), and `sendModelKey` in `src/lib/model-key-sender.ts` sends `PUT /account/model` from the browser to `API_PUBLIC_URL`, with the ticket as its bearer token and no cookies. Every submit asks for a new ticket, because the API spends one even on a key the Provider refuses.
- The key lives only in the uncontrolled field and the submit's local variables. It is never held in React state or echoed back, and the field is emptied after every submit, saved or refused. A refusal is shown under the field in the page's own words for its code (`model_key_invalid`, `model_key_not_permitted`, `provider_unavailable`, `model_key_ticket_invalid`); a stored key is shown only as stored.
- `revokeModelKeyAction` calls `DELETE /account/model/key` with the Session, and the page says what revoking stops: no analysis runs until a key is added again.
- The key section says, before the field, that the résumé text is read by the chosen model when it is uploaded, under the Candidate's own agreement with the Provider.
- The page also reads the Profile, and the summary's primary action goes where the journey opens once a key is stored (`nextStepActionOf`, through `stepAfterModelChoiceOf`): `/journey/resume` while no Profile is built, `/journey/profile` until it is confirmed, then `/journey/analysis`, each with its own label and hint. Until a key is stored it is a disabled button with its reason beside it.
- `API_PUBLIC_URL` is the API's address as the browser reaches it. It is read on the server at request time and handed to the page as a prop, so one image serves every environment, and it defaults to `http://localhost:3001`; the compose stack sets it to the host's API port, as `S3_PUBLIC_ENDPOINT` is set for storage. The API answers CORS for `PUT /account/model` from `WEB_ORIGIN` only.

### Analysis progress (FR-12, TC-04)

The wait while a Curation runs follows the progress component of [Design: AI Analysis](https://github.com/braydevkin/helpmegethired/wiki/Design-AI-Analysis). The `AnalysisProgress` organism takes a `CurationProgress` from `GET /profile/curation` and renders only what the payload holds: the API's percentage, `{saved} of {total} saved`, the pass being read, a window of five `CurationUnitRow`s around it (the saved pass before it stays in view), the career length and counts the model was given, and the footnote that the tab can be closed. It never advances between two answers, so a reload shows the same figure. A Curation back in `queued` with a `resumeAfter` is rendered as paused, by a provider rate limit or by the Account's daily analysis allowance as its `pauseReason` says, with the time it resumes (and the date, for the allowance, which resets at midnight UTC) as a status a screen reader announces, written by `LocalTime` in the browser because the server cannot know the Candidate's time zone; a failed pass says what happened in plain words. No duration, token count, or cost is shown (#102).
...
### Analysis page (FR-12, TC-06)

`/journey/analysis` follows [Design: AI Analysis](https://github.com/braydevkin/helpmegethired/wiki/Design-AI-Analysis) and shows one of four states, decided by the API's answers and never by the page. `AnalysisStep` reads the Profile on the server: an unconfirmed one renders the gate, which counts the review flags still open (`gateNoticeOf`) and leads back to the Profile. A confirmed one reads `GET /profile/curation` and `GET /profile/curation/statements` together. `progress: null` means no Model Key is stored, because confirming and storing a key are the two things that create a Curation, so the page sends the Candidate to `/journey/ai`. Since the key is stored before the upload, this is the state of a key revoked before confirming, or of an Account from before that order. Any other answer hands both payloads to `AnalysisFlow`, a client component that polls with `useCurationProgress`.

- **Running** (`queued` or `running`): `AnalysisProgress` in its page variant (the pass being read and the percentage), `CurationUnitsPanel` with every pass and its kind (Role, Project, Across all, Summary), `CountedFacts` from the metrics the model was given, `RunDetails` (the résumé, the confirmation time through `LocalTime`, the passes left, and the provider with the pinned model), and **Stop the analysis**, which calls `POST /profile/curation/cancel`.
- **Failed or cancelled**: `FailureBanner` names the pass that failed and its reason in plain words (`failureOf`), or asks for a new key after `model_key_rejected`. **Try again** calls `POST /profile/curation/retry`. A retry refused with `curation_model_changed` or `curation_not_retryable` offers **Run it again** instead.
- **Completed**: `StatementList` shows each Statement with its labels, its quoted Evidence, and the Experience or Project it came from, with **Looks right** and **Reject** (`aria-pressed`, pressing the chosen answer again clears it) through `PUT /profile/curation/statements/:id/review`. A rejected Statement is marked as not used. **Run it again** calls `POST /profile/curation/rerun`. The page does not repeat the API's gating rules: it reads them from the progress answer's `rerun`, so a re-run the API would refuse is disabled before any click with the reason for its refusal code beside it, and a refusal that raced a change is shown the same way.
- While a re-run is in flight the Statements of the last completed Curation stay listed with a note, and a run that completes while the page is open reads its Statements once.
- The Job Description entry point (`LockedEntry`) is a focusable `aria-disabled` button whose description says why it is closed. It stays closed after completion until the job matching step exists.
- Server actions in `src/app/(journey)/journey/analysis/actions.ts` are the page's only way to the API, through `CurationClient` and `StatementClient`, which share `jsonBodyOf` and `apiErrorCodeOf`. A refusal keeps its `CurationActionErrorCode` and gets the copy from `refusalMessageOf`. The pure rules live in `src/lib/curation-analysis/view.ts`. No cost, token count, or duration is shown anywhere (#102).
...
`done` means the Profile is saved, not only the text extracted: the record stays `processing` while the Ingestion runs, and the upload page derives its "Building your profile" stage from the Ingestion's Progress. A `failed` record carries one error code, which the page turns into a message: `not_pdf`, `too_large`, `too_many_pages`, `encrypted_pdf`, `corrupt_pdf`, `scanned_pdf`, `upload_incomplete`, `extraction_failed` (retries exhausted on a transient error), and `profile_build_failed` (the Ingestion exhausted its attempts). `POST /resumes` refuses only an Account with no usable Model Key (`409 model_key_missing`, from `ModelChoiceService.usableModelKey`, before anything is read or written, since the Model reads the Resume); otherwise it never refuses a new upload; `complete` answers `409 ingestion_active` while the Account has another record `uploaded` or `processing` or an active Ingestion (TC-05). Uploading the same bytes twice (same Account, same SHA-256) answers the existing record and creates no second job. Both rules are indexes, not only checks: `uploaded_resumes_one_live_per_file_idx` is unique on `(account_id, sha256)` while the record is not `failed` or `expired`, so a rejected file can be uploaded again, and `uploaded_resumes_one_in_flight_per_account_idx` is unique on `account_id` while the record is `uploaded` or `processing`, so two concurrent `complete` calls cannot both pass. The `resumes` module maps its errors to HTTP in one exception filter: not found is `404`, and the two conflicts carry their code in the `ApiError` body.
...
5. **Hand over** through `ExtractionHandover`: the stored text becomes one Segment per Profile part, the Ingestion is inserted with `source = upload` and linked to the record (`ingestion_id`) in the same transaction, and its job is added after the commit. The record stays `processing`; a job re-delivered after the link finds it and never starts a second Ingestion.
...
#### Segments per Profile part

Whenever the cleaned text has a non-blank line, `resumeSegmentsOf` makes exactly one Segment per Profile part, in this order: `header` (headline, summary, LinkedIn URL, GitHub URL), `experience`, `education`, `project`, `skills`, `languages`, `certifications`, each over the whole text. The headings a résumé uses do not say where its entries begin and end: a wrapped bullet can read like a job heading, a sub-heading inside a job like the projects heading, and a certification can sit under the education heading. So no part is cut out of the text before it is read, and every part is looked for everywhere. A résumé that lacks a part still has its Segment, which saves no entry (the header saves a Basic Profile with empty fields, so the Profile can still be confirmed).

A Segment's `input` names the Uploaded Resume and the line range of the cleaned text it covers, the whole text, never the text itself, so Candidate data lives in one place. Each kind has a `SegmentProcessor`: `read` loads the stored text, cleans it, and answers its lines; `recognize` reads them for that kind (below) and answers every field with its Confidence, which is what the Segment keeps; `save` writes the typed rows tagged with the Account, the source Ingestion, and the Segment, replacing what the same Segment wrote before, so a save that runs again after a crash never doubles a row. The upload page's Profile data checklist ticks each part's rows as the `save` Step of its Segment completes; the Progress names the saved kinds only, so the rows say a part was read, never how many entries it holds.

The rules read each part their own way over the whole text, with the section splitter (parser foundation), exactly as `parseResume` reads a draft: the Experiences, the Education, and the Projects from the own lines of their sections (a labelled paragraph of another kind, such as `Idiomas: …` under the summary, left out), the Languages and the Certifications from their sections and from every paragraph labelled with their kind elsewhere, the Skills from the skills section and every technology named in an Experience or a Project. The header alone reads only the text before the first heading, the contact block, and the summary sections, so a link written further down, such as a Project's repository, is never the Candidate's own; `parseResume` still reads the contact over the whole text for the corpus.
...
#### Recognition verified by the rules

The rules alone miss companies, descriptions, and dates, and cut entries where a résumé's layout misleads them, so the Candidate's Model reads each Segment too (ADR-0025). `ResumeSegmentProcessor.recognize` runs the kind's rules first, then asks `SegmentModelReader` (`apps/api/src/recognition`) for the Model's reading: with the Account's usable Model Key it sends the whole cleaned résumé, the kind, and the chosen Model to `RecognitionModel` and logs the token counts; without a key (`ModelKeyNotFoundError`) it answers nothing and the rules' reading is kept as it is, which is what the corpus measures. A failed Model call throws, so the Ingestion's attempts apply: a rate limit spends an attempt instead of pausing the Ingestion, and a key the Provider refuses fails every attempt, so the Profile build fails rather than falling back to the rules. The prompt (`recognition-prompt.ts`, `RECOGNITION_PROMPT_VERSION`) asks the Model to read the whole résumé and answer only the entries of its kind wherever they sit, whatever heading they are written under: an entry runs from where its own heading begins to where the next one begins, a wrapped line continues the bullet above it, a description is the entry's own text copied as written with its sub-headings, a job under a projects heading is a job, a certification under an education heading is a certification, and a technology is never a spoken language. The text sent is capped at `RECOGNITION_INPUT_MAX_CHARACTERS` (30,000, sized for a whole résumé and separate from the Curation Unit cap), cut at a line boundary with `truncated` known. The kind's verifier in `apps/api/src/recognition/verification` (pure functions) turns both readings into the same recognized shape:

- **Grounding.** A value is kept only when its `quote` is in the Segment's text, compared blind to case and whitespace but not to accents, over the non-blank lines joined so a quote may cross lines, and never starting or ending inside a word. A text value must also be said by its own quote word for word (blind to case, accents, and punctuation), so a real quote cannot carry an invented value or one read elsewhere. A period that ends before it starts and a URL that is not `http` or `https` are discarded. A headline or summary quoted from a contact line, or equal to the name the rules read, is discarded, so name, e-mail, and phone still never reach a Profile table. An entry whose naming field (role, institution, name) is not grounded is dropped whole.
- **Alignment.** Entries of a list part are paired one to one, best score first, from a score of 0.5: for Experiences, Education, and Projects the higher of the word overlap of their names (Dice, blind to order, so a heading split the wrong way round still pairs) and the overlap of the lines they sit on (Jaccard, so a heading taken from a bullet still pairs); for Languages and Certifications, which share lines, the word overlap of the names alone; Skills by their canonical name, so `C`, `C++`, and `C#` stay apart. A tie goes to the entries that sit on the same lines. Since the text is the whole résumé, the same role or company may be written at two entries: an entry sits where its rarest text is written (its period or its description, usually), the first time after the entry before it when that text repeats too, and on the occurrences of its other texts nearest to that place, so two positions under the same role each pair with their own.
- **The Model decides which entries exist.** Once the Model has read a Segment, a list entry only the rules read is left out, for every list part including the Skills: the rules cut entries where the layout misleads them, so a rules entry with no Model counterpart is taken for a mis-cut, not a miss. An entry only the Model read is kept. The rules' order is kept for the paired entries, and a Model-only entry goes before the first paired entry below it. An Account with no Model Key keeps the rules' reading whole.
- **Confidence per field.** Both readings agree (same words; same year; same URL up to scheme, `www`, and a trailing slash; same period, or a quote such as `2014 – 2016` that says the years the rules read): `high`, with the rules' verbatim value. Only the Model read it: `high` when a rule confirms it (the period its quote parses to, a URL written whole in its quote, a year written in its quote, a job title word in the role, an institution word or acronym, a degree term, a level word or CEFR code), `medium` otherwise. They disagree: `low`, keeping the rules' period, year, and URL and the Model's text. Only the rules read a field of an entry the Model returned: the rules' value and Confidence. A skill the technology dictionary knows, and its quote names, takes its canonical name and category at `high`; one it does not know keeps the Model's spelling and category at `medium`. The name and e-mail comparison with the Account stays the rules' alone.
...
**Reading the stored text again.** The PDF is deleted once its text is stored, so `POST /profile/recognition` rebuilds the Profile from that text with the Candidate's Model instead of a new upload. It finds the Uploaded Resume behind the latest completed Ingestion and starts a new `upload` Ingestion over `resumeSegmentsOf(raw_text)`, in the transaction that takes the Uploaded Resume back to `processing` linked to it, with its error and finish time cleared: from there the upload page follows it through `GET /resumes/:id`, and completion and failure take the paths above, the replaced rows taking the corrections and the confirmation with them and a current Curation being superseded. The link moves to the new Ingestion, so the Uploaded Resume behind an earlier one is found through its Segments, which name the Uploaded Resume they read; that keeps the Profile's source, and a retry, after a failed reading. It refuses with `409 model_key_missing` without a Model Key, `409 ingestion_active` while an Ingestion is queued or running or another upload is in flight, `409 curation_active` while a Curation is queued or running (checked under the Account lock the Ingestion takes), and `404 resume_text_missing` when no Profile has been built or its Uploaded Resume has no text. The Profile page offers it only while a Model Key is stored, behind a confirmation that says what is replaced, and moves to `/journey/resume` once it starts.
...
**Corrections.** The Candidate's Model reads the résumé and the rules verify what it read, which still leaves it wrong sometimes, and `/profile/parts` is the Candidate's answer: `PUT /profile/parts/basic-profile` replaces the four Basic Profile fields, and `POST /profile/parts/{part}`, `PUT /profile/parts/{part}/{id}` and `DELETE /profile/parts/{part}/{id}` add, correct and remove one entry of a list part. Each answers the whole Profile, so the page re-renders from one payload. A correction is written on the row the Ingestion built, keeping its `source_ingestion_id`, which means the next Ingestion replaces it with everything else; an entry the Candidate adds belongs to no Segment (`segment_id` is null) and goes to the end of its part. Every write records `edited_at`, which is what drops the entry's review flags: a flag survives only while an untouched row still carries the name it was flagged under, and the flags that name no entry go once the Basic Profile is corrected (`flagsStillOpen`). Confirming closes the surface: every write route answers `409` afterwards, because the confirmation starts the Curation that reads the Profile. The part owns its own payload schema (`PROFILE_ENTRY_CORRECTION_SCHEMAS` in `packages/shared`), so nothing else has to know what an Experience or a Certification may say. The schemas with a period also refuse one that ends before it starts, on `period.end`; the rule holds only for corrections, because a résumé with reversed dates is still read as it was written.
...
| Route | Answers |
| --- | --- |
| `POST /resumes` (file name, size, SHA-256, content type) | `201` with the record, the presigned `PUT` URL, and its expiry; `200` with the existing record when the same bytes are already `uploaded`, `processing`, or `done`; a `pending` duplicate gets a fresh URL; `409 model_key_missing` with nothing reserved while the Account has no usable Model Key |
...
| `POST /profile/recognition` | `202` with the `uploadedResumeId` whose stored text a new Ingestion reads with the Candidate's Model, the record back to `processing`; `409 model_key_missing`, `409 ingestion_active`, `409 curation_active`; `404 resume_text_missing` while no Profile with stored text exists |
| `PUT /profile/parts/basic-profile` | `200` with the Profile, the Basic Profile replaced by what the Candidate says is true; `409` once confirmed |
| `POST /profile/parts/{part}` | `200` with the Profile, one entry added at the end of the part, belonging to no Segment; `400` on an entry that breaks its part's schema; `409` once confirmed |
| `PUT /profile/parts/{part}/{id}` | `200` with the Profile, the entry replaced in place; `404` when no such entry belongs to the Account; `409` once confirmed |
| `DELETE /profile/parts/{part}/{id}` | `200` with the Profile, the entry gone; `404` when no such entry belongs to the Account; `409` once confirmed |
...
Model Key stored, Profile confirmed ──┘                                            │ after commit
             ...
             every unit not yet saved, three at a time, the synthesis unit last:
               computed facts + unit input (≤ 8,000 characters) ──▶ model ──▶ schema ──▶ Evidence resolves?
                   ...
                   the embedding ceiling ──▶ resume_after at the next UTC midnight, queued, the attempt kept
```
...
| `running` | `queued` | Worker | A call produced no usable output and `attempts` is below `max_attempts` (retried with backoff), or the Provider rate-limited the Candidate's key or reported an exhausted quota (`resume_after` set, `pause_reason` `provider_rate_limit`, the attempt not consumed), or the Account's embedding reservation was refused at its ceiling (`resume_after` the next UTC midnight, `pause_reason` `embedding_ceiling`, the attempt not consumed) |
...
- **Input.** Each unit's subject, capped at 8,000 characters and cut at a line boundary, with `truncated` recorded when the cap applied, inside the delimiters the security rules require. The deterministic facts computed by the pure metrics module (career duration, duration per company, the counts of roles, projects, certifications, languages, and education; a Project records no period, so it has no duration) sit outside the Candidate block as computed facts, so the model judges and never counts. The synthesis unit reads the Statements the other units saved.
...
A Curation turns only Experience and Project descriptions into Statements, so what a Requirement such as "5 years of experience", "a degree in computer science", or "fluent English" asks for would otherwise reach a Job Analysis only as counts. When a Curation completes, the same transaction records its Facts (`facts`): the years of experience, counted across every Experience with the career duration merge the metrics use, and one row for each Education, Certification, Language, and Skill of the confirmed Profile, worded by code as `Education: MSc in Computer Science, University of Cambridge, 2014-01 to 2016-12` and naming the Profile row it restates in `source_id`. No Model call is made, so a Fact cannot drift or reword a degree (ADR-0026). The Facts belong to the Curation and go with its Statements when it is superseded. A Curation completed before Facts existed gets them once from the reconciliation job, which records them by code for every completed Curation that has none; every Curation records at least its years of experience, so the next run finds nothing left to do. `factsOf` in `curation/facts-of.ts` is the pure derivation and `FactRepository.ofCuration` the Account-scoped read a Job Analysis will make.

#### Embedding and retrieval

Once every unit is saved, every Statement of the Curation is embedded through the platform embedding model into `vector(1536)` (ADR-0023), and `completed` is written in the same transaction as the last vectors, before the job returns. An embedding failure is a failure of the Curation, not of a unit: attempts and backoff apply, and no Statement is left half-indexed. Before the call, `EmbeddingAllowanceRepository.reserve` counts the Statements' estimated tokens against the Account's daily ceiling in `embedding_usage`, in one transaction under the Account lock. A refused reservation makes no call and pauses the Curation with `pause_reason` `embedding_ceiling` and `resume_after` at the next UTC midnight, keeping its attempt, rather than failing it; reconciliation enqueues it again after that time. The ceiling, its argument, and the per-Account override are in [security.md, "Spend"](security.md#spend) (#133).
...
- `selectCurationModel` and `selectEmbeddingModel` choose the adapter from platform configuration, the way `selectCodeSender` chooses the email sender (ADR-0018). Unset outside production selects the deterministic fake: output derived from its input, valid against the shared schemas, with Evidence that resolves, a deterministic 1536-dimension vector, and a failure or a schema-invalid response on demand, so every failure path is testable without a Provider. A Model Key shaped `sk-ant-fake-<outcome>-on-<unit kind>` scripts the fake for that one Candidate: every unit of that kind plays the outcome, recognised by the instructions `instructionsFor` gives it, and every other call is answered, which is how the end-to-end suite drives a single Curation into a failure or a rate-limit pause on a shared stack. Only the fake reads the key that way, and a production configuration never selects the fake. A production configuration without the platform embedding key refuses to start.
...
| Wait out a pause | `queued` with `resume_after` in the future, after a Provider rate limit or at the embedding ceiling, read from the job's `Clock` | skipped; enqueued on the first run after that time |
...
| `GET /profile/curation` | The current Curation, the newest one of the latest completed Profile that is not `superseded`: status, the percentage `floor(100 × saved units ÷ units)`, the total and saved counts, every unit in order with its kind, title, and status (the units being read are the `running` ones, up to three at once), the computed facts, the Model in use, the failure reason, `resume_after` with its `pauseReason` (`provider_rate_limit` or `embedding_ceiling`, `null` when the Curation is not paused), and `rerun`: whether a re-run is `allowed`, and the `refusal` code `POST /profile/curation/rerun` would answer (`curation_active`, `curation_not_ready`, or `curation_unchanged`); an `ETag` over the whole answer, so a change of Model Choice that makes a re-run available changes it and `304` on `If-None-Match`; `{ "progress": null }`, not a `404`, when the Account has no such Curation. Statements are not part of it |
...
The `e2e` package depends on `@helpmegethired/web`, so `pnpm turbo run test:e2e` builds the web app first and Playwright starts it with `next start` on port 3100. Setting `E2E_BASE_URL` points the tests at an already running stack instead, and `E2E_API_URL` (default `http://localhost:3001`) names the API for the scenarios that call it directly. `resume-upload.api.spec.ts` signs in through the web app, reads the Session cookie, takes a synthetic corpus PDF to a confirmed Profile and, with a Model Key stored, to a completed Curation with Statements, and takes a hostile file to `failed` with its code while the Profile and its Curation stay. `curation.api.spec.ts` covers the Curation paths with keys that script the fake: a synthesis unit failing past its attempts ends the Curation `failed`, and a retry resumes it with every saved unit left saved; a new Resume supersedes a completed Curation and none of its Statements return; and a new Resume supersedes a Curation paused by a rate limit. Both run against the compose stack in CI with no provider key. Browsers are installed once with `pnpm --filter e2e exec playwright install chromium`.
...
- The `web` container receives `API_URL=http://api:3001`, and `API_PUBLIC_URL` on the host's API port for the Model Key the browser sends, and starts only after `api` is healthy. When the web app runs natively, `API_URL` defaults to `http://localhost:3001`.
