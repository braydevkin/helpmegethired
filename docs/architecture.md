...
- The Profile step reads the Profile on the server and renders the review as the design has it, with the Candidate's corrections on top of it (#178). `BasicProfileCard` carries the headline, summary and the two URLs; `ExperienceCorrections` puts `Correct this role`, `Remove` and `Add a role` on the timeline, opening one entry at a time. The forms are corrections of the Experience the API answered, never of the period the timeline writes for reading, and `correction-form.ts` turns the fields back into the shared schemas before anything is sent, so a message lands on the field it belongs to. Every save is a server action that answers the whole Profile through `revalidatePath`, so the step re-renders from the API and never from local state; a confirmed Profile renders with no correction offered, because the write routes answer `409`.
