...
5. **Hand over** through `ExtractionHandover`: the section splitter creates the Segments, the Ingestion is inserted, and its job is added (#79). Until that lands, the handover marks the record `done`.
...
| Experiences | Date range detection in both languages (month names and abbreviations, `MM/YYYY`, a year alone spanning January to December, `since`/`desde`, open endings), a line with a range starts an entry: its heading is the one or two short non-sentence lines before it, or the rest of the range line when the dates come first; what follows up to the next heading is the description. A short line naming a role and a company after a sentence opens an entry without dates. Title and company split on the usual separators with a job title dictionary deciding which side is the title (`high`), both kept raw otherwise (`low`); a heading with a single part names a role and no company. Career months from the periods with overlaps and adjacent ranges merged, both ends inclusive, open ranges ending today; exposed as a pure function `GET /profile` reuses for the years of experience. Two-column interleaving still breaks headings until the regrouping lands |
