...
The rules alone miss companies, descriptions, and dates, so the Candidate's Model reads each Segment too. `ResumeSegmentProcessor.recognize` runs the kind's rules first, then asks `SegmentModelReader` (`apps/api/src/recognition`) for the Model's reading: with the Account's usable Model Key it sends the lines the kind's rules read (the Segment's cleaned lines, without a labelled paragraph of another kind), its kind, and the chosen Model to `RecognitionModel` and logs the token counts; without a key (`ModelKeyNotFoundError`) it answers nothing and the rules' reading is kept as it is, which is what the corpus measures. A failed Model call throws, so the Ingestion's attempts apply: a rate limit spends an attempt instead of pausing the Ingestion, and a key the Provider refuses fails every attempt, so the Profile build fails rather than falling back to the rules. The kind's verifier in `apps/api/src/recognition/verification` (pure functions) turns both readings into the same recognized shape:
