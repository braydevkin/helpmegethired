...
- **Trigger: `POST /profile/confirm`.** Curating fields the Candidate has not reviewed would anchor Statements in text that is about to be corrected. Confirm stays idempotent: a repeat finds the existing Curation and answers `200`, never `409`. An Account with no Model Key is refused at confirm with a reason it can act on (ADR-0023).
- **Creation in the confirm transaction.** One `curations` row (`queued`) and one `curation_units` row per unit (`pending`): one unit per Experience, one per Project, one cross-cutting unit for competences that appear in more than one place, and one synthesis unit that reads what the others produced. No cap: every Experience and Project is curated. Units are derived at creation so progress has a denominator from the first second.
